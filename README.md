...
Afterwards please reference the actual `README.md` you just created in your new project folder, all the references to kikoha will be changed accordingly.
...
*Prerequisites: virtualenv, virtualenvwrapper*
    ...
    cd kikoha
    mkvirtualenv kikoha-dev && add2virtualenv `pwd`
    mkvirtualenv kikoha-prod && add2virtualenv `pwd`
    mkvirtualenv kikoha-test && add2virtualenv `pwd`
...
*`.env.dev` is not version controlled so the first person to create this project needs to create a `.env.dev` file for Foreman to read into the environment. Future collaboraters need to email the creator for it.*
    ...
    echo PYTHONPATH=kikoha >> .env.dev
...
**WARNING**: If this project has SSL turned on, [localhost:5000](http://localhost:5000) won't work anymore because it will always try to redirect to [https://localhost:5000](https://localhost:5000). To fix this comment out the SECURITY CONFIGURATION section in `/kikoha/config/settings/prod.py`
...
*`.env.test` is not version controlled so the first person to create this project needs to create a `.env.test` file for Foreman to read into the environment. Future collaboraters need to email the creator for it.*
    ...
    echo PYTHONPATH=kikoha >> .env.test
...
Grunt automatically compiles Jasmine tests written in CoffeeScript at `/kikoha/static/js/tests/coffee` and runs the tests upon every save.
...
SSL
---
Enable SSL via Heroku, Cloudflare, or your DNS provider and then uncomment the SECURITY CONFIGURATION section in `/kikoha/config/settings/prod.py` to enable django-secure and other security best practices for production.
